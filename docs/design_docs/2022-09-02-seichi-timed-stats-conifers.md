...
- 起動後は、サーバーは二つのフェーズを交互に繰り返す
  - スナップショット作成フェーズ。以下の動作を、数日程度の間隔で繰り返す：
    - 統計量スナップショットを作成し、RDB の「スナップショットデータテーブル」に書き込む
  ...
  - 差分記録フェーズ。5 分程度の短い間隔で、次回の「スナップショット作成フェーズ」まで以下の動作を繰り返す：
    ...
    - RDB の「差分データテーブル」に `diff` を書き込む
    - `snap` を「前回の統計量スナップショット」とする

#### どのように差分データを永続化するかの詳細

(TODO: 「データ点テーブル」と「スナップショット点テーブル」が用意されて、そこに時刻やほかのデータ点との関連性などのメタデータが保存されることを述べる)

#### 具体的なスキーマ

差分データでは UUID が占める容量が大きくなると想定されるため、UUID をより小さい `bigint` に落とす `uuid_mapping` テーブルを用意する。

整地量を保存するテーブルは、[このような設計](https://github.com/GiganticMinecraft/seichi-timed-stats-conifers/blob/1c0ec13e4b26bb2e07418f393ca5a5a72feebb4a/database/schema.sql#L7-L44)になる。 (TODO: スナップショットデータテーブルがこれには無いので付け加える必要がある！)
