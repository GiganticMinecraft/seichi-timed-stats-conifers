...
FROM lukemathwalker/cargo-chef:0.1.41-rust-1.63.0 AS chef
WORKDIR /app
...
FROM bufbuild/buf:1.19.0 as buf
...
RUN --mount=type=cache,target=/app/server cargo chef cook --release --recipe-path recipe.json

# Build application
COPY --link . .
RUN --mount=type=cache,target=/app/server cargo build --release
